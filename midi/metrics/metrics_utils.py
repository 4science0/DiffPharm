...
def phar_type_counts(data_list, num_classes):
    print("Computing node pharmacophore types distribution...")
    counts = np.zeros(num_classes)
    for data in tqdm(data_list, total=len(data_list)):
        x = torch.nn.functional.one_hot(data.cx, num_classes=num_classes)
        try:
            assert data.cx_sup.shape == data.cx.shape
        ...
        for i in range(data.num_nodes):
            ...
            for j in neighbors:
                for k in neighbors:
                    ...
                    norms = torch.norm(a) * torch.norm(b)
                    # Handle edge case for zero norm to avoid division by zero
                    if norms == 0:
                        print(f"Zero norm detected for atom pair: i={i}, j={j}, k={k}, skipping...")
                        continue

                    # print(a, b, torch.norm(a) * torch.norm(b))
                    angle = torch.acos(dot_product / (norms + 1e-6))  # Add small epsilon for stability
                    angle = angle * 180 / math.pi
                    
                    # Check for NaN angle
                    if torch.isnan(angle):
                        print(f"NaN angle detected for atom pair: i={i}, j={j}, k={k}, skipping...")
                        continue
