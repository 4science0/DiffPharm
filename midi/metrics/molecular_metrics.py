...
from utils import get_template_sdf
...
class SamplingMetrics(nn.Module):
    def __init__(self, train_smiles, dataset_infos, test, template=None, template_name=None, test_smiles=None):
        super().__init__()
        ...
        self.filter_validity_metric = MeanMetric()
    ...
    def compute_validity(self, generated):
        ...
        all_invalid_mols = defaultdict(list)
        ...
        for mol in generated:
            ...
            if rdmol is not None:
                try:
                    ...
                    if match:
                        all_invalid_mols[rdmol.GetProp('template_idx')].append(largest_mol)
                        error_message[4] += 1
                        continue
                    
                    filter_valid.append(smiles)
        ...
        print(f"Error messages: AtomValence {error_message[1]}, Kekulize {error_message[2]}, other {error_message[3]}, "
              f" -- No error {error_message[-1]},  invalid mols: {error_message[4]}")
        self.validity_metric.update(value=len(valid) / len(generated), weight=len(generated))
        self.filter_validity_metric.update(value=len(filter_valid) / len(generated), weight=len(generated))
        ...
        return valid, connected_components, all_smiles, all_valid_mols, error_message, all_invalid_mols
    ...
    def evaluate(self, generated, local_rank):
        ...
        # Validity
        valid, connected_components, all_smiles, all_valid_mols, error_message, all_invalid_mols = self.compute_validity(generated)

        validity = self.validity_metric.compute()
        filter_validity = self.filter_validity_metric.compute()
        ...
        print(f"Validity over {num_molecules} molecules:"
              f" {validity * 100 :.2f}%")
        print(f"Filter Validity over {num_molecules} molecules:"
              f" {filter_validity * 100 :.2f}%")
        ...
        if wandb.run:
            dic = {'Validity': validity,
                   'Filter Validity': filter_validity,
        ...
        result_dict = {
            ...
            'Filter Validity': filter_validity.item()*100,
        ...
        return all_smiles, all_valid_mols, result_dict, all_invalid_mols
    ...
    def __call__(self, molecules: list, name, current_epoch, local_rank, sample_type="mixed_sample"):
        ...
        all_generated_smiles, all_valid_mols, result_dict, all_invalid_mols = self.evaluate(molecules, local_rank=local_rank)
        ...
        if len(all_valid_mols.keys())>0:
            ...
            for idx in all_valid_mols.keys():
                ...
                if template is None or isinstance(template, type(None)):
                    template = Chem.RWMol()
        ...
        result_path2 = os.path.join(current_path, f"{result_dir}/epoch{current_epoch}/", 'invalid_mols')
        os.makedirs(result_path2, exist_ok=True)
        filename2 = f"{result_path2}/final_smiles_GR{local_rank}_{0}.txt"
        sdf_filename2 = f"{result_path2}/final_mol_GR{local_rank}_{0}"
        for i in range(2, 10):
            if os.path.exists(filename2):
                filename2 = f"{result_path2}/final_smiles_GR{local_rank}_{i}"
                sdf_filename2 = f"{result_path2}/final_mol_GR{local_rank}_{i}"
            else:
                break
        if len(all_invalid_mols.keys())>0:
            self.validity_flag = True
            for idx in all_invalid_mols.keys():
                smiles_set = set()
                sample_i = 0
                template = self.template_dict.get(idx)
                if template is None or isinstance(template, type(None)):
                ...
                smiles_f2 = open(f"{filename2}_template{idx}.txt", 'w')
                # sdf_filename = os.path.join(sdf_filename, 'invalid_mols')
                # os.makedirs(sdf_filename, exist_ok=True)
                with Chem.SDWriter(f"{sdf_filename2}_template{idx}.sdf")as f:
                    f.write(template)
                    for i, mol in enumerate(all_invalid_mols[idx]):
                        if mol.GetProp("smiles") not in smiles_set:
                            mol.SetProp('_Name', f"sample{sample_i}")
                            f.write(mol)
                            smiles_f2.write(mol.GetProp('smiles')+'\n')
                            sample_i +=1
                            smiles_set.add(mol.GetProp("smiles"))

                smiles_f2.close()
            print(f'All invalid mol saved on rank {local_rank}')
