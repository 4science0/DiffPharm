...
from models.layers import PositionsMLP, PositionsMLP_Control
from models.transformer_model import XEyTransformerLayer, GraphTransformer
...
class ControlNet(nn.Module):  
    ...
    def forward(self, data: utils.PlaceHolder):
        ...
        pos = self.mlp_in_pos(data.pos.float(), node_mask) + self.control_in_pos(data.cpos.float(), node_mask)  # bs, n, 3
        if self.dropout_rate>0:
...
class ControlGraphTransformer(GraphTransformer): 
    ...
    def forward(self, data: utils.PlaceHolder, control_data: List[utils.PlaceHolder]):
        ...
        features = utils.PlaceHolder(X=self.mlp_in_X(X), E=new_E, y=self.mlp_in_y(data.y), charges=None,
                                     pos=self.mlp_in_pos(data.pos.float(), node_mask), node_mask=node_mask).mask()
