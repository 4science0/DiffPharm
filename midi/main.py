...
from datasets import geom_phar_dataset, chembl_geom_dataset, drugs_geom_dataset, zinc_dataset
...
def get_resume(cfg, dataset_infos, train_smiles, checkpoint_path, test: bool):
    ...
    model = FullDenoisingDiffusion.load_from_checkpoint(checkpoint_path, map_location={'cuda:1': 'cuda:0'}, weights_only=False,
...
def load_pretrained_model(model, pretrained_model_path):
    pretrained_weights = torch.load(pretrained_model_path, map_location={'cuda:1': 'cuda:0'}, weights_only=False)
...
def main(cfg: omegaconf.DictConfig):
    ...
    datamodule = None
    if dataset_config.name in ["geom_drugs", "chembl_geom", 'zinc']:
        if dataset_config.name == 'geom_drugs':
            datamodule = geom_phar_dataset.GeomDataModule(cfg)
            dataset_infos = geom_phar_dataset.GeomInfos(datamodule=datamodule, cfg=cfg)
        if dataset_config.name == 'chembl_geom':
            datamodule = chembl_geom_dataset.GeomDataModule(cfg)
            dataset_infos = chembl_geom_dataset.GeomInfos(datamodule=datamodule, cfg=cfg)
        if dataset_config.name == "zinc":
            datamodule = zinc_dataset.LargeZincDataModule(cfg)
    ...
    trainer = Trainer(gradient_clip_val=cfg.train.clip_grad,
                      strategy=strategy,  # Needed to load old checkpoints
                      reload_dataloaders_every_n_epochs=1,  # 🔥 每轮重新调用 datamodule.train_dataloader()
                      ...
                      callbacks=callbacks,
                      log_every_n_steps=5 if name != 'debug' else 1,
                    #   max_time="21:00:00:00",
                      # num_nodes=num_nodes
                      )
