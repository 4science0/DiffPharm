...
def to_dense(data, dataset_info, device=None): 
    ...
    try:
        cX_sup,_ = to_dense_batch(x=data.cx_sup, batch=data.batch)
    except:
        cX_sup = None
    ccharges, _ = to_dense_batch(x=data.ccharges, batch=data.batch)
    
    cmax_num_nodes = cX.size(1)
    cedge_index, cedge_attr = remove_self_loops(data.cedge_index, data.cedge_attr)
    cE = to_dense_adj(edge_index=cedge_index, batch=data.batch, edge_attr=cedge_attr, max_num_nodes=cmax_num_nodes).long()
    cpos, _ = to_dense_batch(x=data.cpos, batch=data.batch)
    
    
    cpos = cpos.float()
    if dataset_info is not None:
        cX, ccharges, cE = dataset_info.to_one_hot(X=cX, charges=ccharges, E=cE, node_mask=node_mask, X_sup=cX_sup, just_control=True)
    else:
        cX, ccharges, cE = to_one_hot(X=cX, charges=ccharges, E=cE, node_mask=node_mask, X_sup=cX_sup, just_control=True)
    ...
    return data.mask(just_control=True)


def to_dense2(data, dataset_info, device=None, control_data=None): 
    DATA = deepcopy(data)
    NODE_MASK = None
    if control_data:
        data = control_data
    X, node_mask = to_dense_batch(x=data.x, batch=data.batch)
    NODE_MASK = deepcopy(node_mask)
    data.x = data.cx
    pos, _ = to_dense_batch(x=data.pos, batch=data.batch)
    pos = pos.float()
    assert pos.mean(dim=1).abs().max() < 1e-3
    ...
    if control_data:
        data = DATA
    cX, node_mask = to_dense_batch(x=data.cx, batch=data.batch)
    try:
        cX_sup,_ = to_dense_batch(x=data.cx_sup, batch=data.batch)
    except:
        cX_sup = None
    ...
    data = PlaceHolder(X=X, charges=charges, pos=pos, E=E, y=y, node_mask=node_mask,
                       cX=cX, ccharges=ccharges, cpos=cpos, cE=cE, cy=cy, idx=data.idx)
    return data.mask(just_control=True, node_mask=NODE_MASK)

def control_to_dense(data, dataset_info, phar_sup=None, device=None): 
    cX, node_mask = to_dense_batch(x=data.cx, batch=data.batch)
    max_num_nodes = cX.size(1)
    if phar_sup:
        cX_sup,_ = to_dense_batch(x=data.cx_sup, batch=data.batch)
    else:
        cX_sup = None
    ...
    cedge_index, cedge_attr = remove_self_loops(data.cedge_index, data.cedge_attr)
    cE = to_dense_adj(edge_index=cedge_index, batch=data.batch, edge_attr=cedge_attr, max_num_nodes=max_num_nodes).long()
    
    has_nonzero = torch.any(cE != 0)
    print('there are edges in control information') if has_nonzero else print('No edges in control information')
    ...
    data = PlaceHolder(X=cX, E=cE, cX=cX, ccharges=ccharges, cpos=cpos, cE=cE, cy=cy, node_mask=node_mask, idx=data.idx)
    return data.mask(just_control=True)
...
class PlaceHolder:
    ...
    def mask_no_mean(self, node_mask=None, just_control=False):
        if node_mask is None:
            assert self.node_mask is not None
            node_mask = self.node_mask
        node_mask = node_mask.to(self.X.device)
    ...
    def add_scales(self, feature, node_mask=None):
        X = self.X + feature.X
