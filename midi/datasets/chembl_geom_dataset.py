...
from datasets.dataset_utils import load_pickle, save_pickle, reconstruct_data_list
...
full_phar_encoder = {'Linker': 0, 'Donor': 1, 'Acceptor': 2, 'NegIonizable': 3, 'PosIonizable': 4, 'Aromatic': 5, 'Hydrophobe': 6}
...
class GeomDrugsDataset(InMemoryDataset):
    def __init__(self, split, root, dataset_cfg, transform=None, pre_transform=None, pre_filter=None, val_template_num=200, test_template_num=200):
        ...
        if len(self.processed_paths) > 10:
            self.template_data = torch.load(self.processed_paths[10], weights_only=False)
    ...
    def processed_file_names(self):
        ...
        if self.split == 'train':
            return [f'train_{h}.pt', f'train_n_{h}.pickle', f'train_atom_types_{h}.npy', f'train_phar_types_{h}.npy', f'train_bond_types_{h}.npy',
                    ...
                    f'train_angles_{h}.npy', f'train_smiles.pickle', f'train_template_{h}.pt', f'train_pharmacophore_statistics_{h}.json']
        elif self.split == 'val':
            return [f'val_{h}.pt', f'val_n_{h}.pickle', f'val_atom_types_{h}.npy', f'val_phar_types_{h}.npy', f'val_bond_types_{h}.npy',
                    ...
                    f'val_angles_{h}.npy', f'val_smiles.pickle', f'val_template_{h}.pt', f'val_pharmacophore_statistics_{h}.json']
        else:
            if self.template_name is None:
                return [f'test_{h}.pt', f'test_n_{h}.pickle', f'test_atom_types_{h}.npy', f'test_phar_types_{h}.npy',f'test_bond_types_{h}.npy',
                        ...
                        f'test_angles_{h}.npy', f'test_smiles.pickle', f'test_template_{h}.pt', f'test_pharmacophore_statistics_{h}.json']
            else:
                ...
                #         f'test_smiles_{self.template_name}.pickle', f'test_template_{h}_{self.template_name}.pt', f'test_pharmacophore_statistics_{h}_{self.template_name}.json']
                return [f'test_{h}.pt', f'test_n_{h}.pickle', f'test_atom_types_{h}.npy', f'test_phar_types_{h}.npy',f'test_bond_types_{h}.npy',
                        f'test_charges_{h}.npy', f'test_valency_{h}.pickle', f'test_bond_lengths_{h}.pickle',
                        f'test_angles_{h}.npy', f'test_smiles.pickle', f'test_template_{h}_{self.template_name}.pt', f'test_pharmacophore_statistics_{h}.json']
    ...
    def process(self):
        ...
        #         for _ in range(3):
        #             data, pharmacophore_statistics = dataset_utils.mol_to_pharmacophore(mol, smiles, full_phar_encoder, full_atom_encoder, pharmacophore_statistics)
        #             if self.remove_h:
        ...
        for i, smiles in enumerate(tqdm(all_files)):
            ...
            for j, conformer in enumerate(all_conformers):
                ...
                if not IA_existence and condition:
                    for _ in range(3):
                        data, pharmacophore_statistics = dataset_utils.mol_to_pharmacophore(conformer, smiles, full_phar_encoder, full_atom_encoder, pharmacophore_statistics)
                        if self.remove_h:
                            data = dataset_utils.remove_hydrogens(data)
                        if self.pre_filter is not None and not self.pre_filter(data):
                            continue
        ...
        with open(self.processed_paths[11], 'w') as f:
            json.dump(pharmacophore_statistics, f, indent=4) 
        ...
        # template_data = data_list[::10][:template_num]

        for i in range(len(template_data)):
            template_data[i].idx = i
        torch.save(template_data, self.processed_paths[10])
        # else:
        #     data_, slices_ = torch.load(self.processed_paths[0])
        #     data_list = reconstruct_data_list(data_, slices_)
